...
from backend.models import Chat, ChatRoom
...
def group_chat_index(request):
    ...
    rooms = Room.objects.all  # Exclude the current user
...
def search_rooms(request):
    if "query" in request.GET:
        ...
        rooms = Room.objects.filter(slug__icontains=query)
...
def send_message(request, receiver_room_slug):
    ...
    receiver_room = Room.objects.get(slug=receiver_room_slug)
...
def chat_history(request, receiver_room_slug):
    messages = ChatRoom.objects.filter(sender=request.user, receiver=receiver_room_slug)
